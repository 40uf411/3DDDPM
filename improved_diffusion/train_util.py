...
class TrainLoop:
    ...
    def run_loop(self):
        while (
            ...
            masks = 0
            self.run_step(batch, masks, cond)
    ...
    def run_step(self, batch, masks, cond):
        self.forward_backward(batch, masks, cond)
        if self.use_fp16:
            self.optimize_fp16()
        else:
    ...
    def forward_backward(self, batch, masks, cond):
        zero_grad(self.model_params)
        for i in range(0, batch.shape[0], self.microbatch):
            micro = batch[i : i + self.microbatch].to(dist_util.dev())
            micro_masks = masks[i : i + self.microbatch].to(dist_util.dev()) if masks is not 0 else 0
            micro_cond = {
